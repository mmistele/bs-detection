...
X_train, Y_train = read_csv('data/train_shrink.csv') 
X_dev, Y_dev = read_csv('data/dev.csv') 

maxLen = max(len(max(X_train, key=len)), len(max(X_dev, key=len)))
print "Max length: %s" % maxLen

counts = get_char_counts_from_csv('data/train_shrink.csv') + get_char_counts_from_csv('data/dev.csv') 
...
model.fit(X_train_indices, Y_train, epochs = 20, batch_size = 200, shuffle=True, callbacks = [tensorboard], validation_split = 0.2)
